...
The `tempfile.mktemp()` function is explicitly marked as unsafe in Python's documentation. The file name returned may not exist when generated, but by the time the model attempt to create it, another process may have created a file with that name.
