...
Every piece of knowledge in a system should have a single, authoritative representation. When that knowledge changes, only one place should require an edit.

## Why This Matters in Production

Duplicated logic is a ticking time bomb. When a business rule changes and one copy is updated while another is missed, inconsistent behavior results; it is hard to detect and harder to debug. The more copies exist, the more likely one diverges silently.

DRY is not about eliminating similar-looking code. It is about eliminating duplicated knowledge — the same business rule, the same decision, or the same source of truth expressed in multiple places.
...
3. **Extract when the pattern is stable.** The first occurrence is not extracted because the right shape of the abstraction is not yet known. Extraction follows at least two repetitions with the same semantics.
...
- **DRY is about knowledge, not syntax.** Two blocks of code that look identical may represent different things. They should not be unified merely because they look the same.
- **Premature DRY creates wrong abstractions.** A bad abstraction is worse than duplication because it is harder to undo. Wait until the pattern is understood before extracting.
...
- **Tension with YAGNI:** Do not build a generic utility "because it might be needed elsewhere." Extract when concrete duplicates exist.
