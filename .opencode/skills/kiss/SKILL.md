...
Given two solutions that produce the same result, the one easier to read, understand, and change is preferred.

## Why This Matters in Production

Simple code survives contact with production. Complex code breaks in ways that are hard to diagnose, hard to fix, and hard to verify without introducing another regression. Every incident response starts with code being read under pressure; when it cannot be understood quickly, the outage gets longer.
...
1. **Optimize for reading, not writing.** Code is read 10x more often than it is written. A few extra lines of clear code are better than a one-liner that requires a comment to explain.
...
4. **One function, one job.** If the function requires the word "and" in its description, split it.
...
- **Simple does not mean naive.** Error handling, input validation, and proper data structures are not "complexity" — they are correctness. Safety must not be stripped away in the name of simplicity.
...
- **Some domains are inherently complex.** Cryptography, distributed consensus, and financial calculations have irreducible complexity. KISS means not adding unnecessary complexity on top.
...
- [ ] Are there comments explaining "what" instead of "why"? (If so, the code itself is not clear enough.)

## Related Skills

- **yagni**: When the complexity comes from building things not yet needed
